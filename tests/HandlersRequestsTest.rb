...
class HandlersTest < MiniTest::Test
	...
	def test_HandlersCreateNonArrFields
		@handlerCollection.each do |e|
			if !e[1].nil? && e[1] == 'requiresIDInConstructor'
				inst = e[0].new(RubyClient.new, 999)
			else
				inst = e[0].new(RubyClient.new)
			end
	...
	def test_PlatformHandlerHasTypes
		assert_instance_of PlatformTypesHandler, PlatformHandler.new(RubyClient.new).types
	end
	def test_PlatformHandlerHasDevices
		assert_instance_of PlatformDevicesHandler, PlatformHandler.new(RubyClient.new).devices
	end
	def test_PlatformHandlerHasOS
		assert_instance_of PlatformOSHandler, PlatformHandler.new(RubyClient.new).os
	end
	def test_PlatformHandlerHasBrowsers
		assert_instance_of PlatformBrowsersHandler, PlatformHandler.new(RubyClient.new).browsers
	end
	def test_UserHandlerHasOrganizations
		assert_instance_of UserOrganizationsHandler, UserHandler.new(RubyClient.new).organizations
	end
