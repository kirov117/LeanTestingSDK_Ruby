...
class EntitiesTest < MiniTest::Test
	...
	def test_EntitiesCorrectParent
		@entityColllection.each { |e| assert_kind_of Entity, e[0].new(RubyClient.new, {'id'=> 1}) }
	end

	def test_EntitiesDataParsing
		data = {'id'=> 1, 'YY'=> 'strstr', 'FF'=> [1, 2, 3, 'asdasdasd'], 'GG'=> {'test1'=> true, 'test2'=> []}}
		@entityColllection.each do |e|
			assert_equal e[0].new(RubyClient.new, data).data, data
		end
	end
	...
	def test_EntitiesHaveSecondaries
		@entityColllection.each do |e|
			next if e[1].nil?
			e[1].each do |sk, sv|
				assert_instance_of sv, e[0].new(RubyClient.new, {'id'=> 1}).instance_variable_get('@' + sk)
			end
		end
	end
